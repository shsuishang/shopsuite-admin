  ...
  >
    <el-form ref="formRef" label-width="80px" :model="form" :rules="rules">
      ...
      <el-form-item :label="t('积分')" prop="points">
        <el-input v-model="form.points" clearable :placeholder="t('积分 - 正数为增加积分，负数为扣减积分积分')"/>
      </el-form-item>
...
<script>
  ...
  import {updatePoints} from '@/api/pay/userResource'
    ...
    setup(props, {emit}) {
      ...
      const save = () => {
        state['formRef'].validate(async (valid) => {
          if (valid) {
            if (state.isUpdate) {
              const { msg, status } = await updatePoints({user_id: state.form.user_id, points: state.form.points})
              if (200 == status) {
                $message(msg, 'success')
              } else {
                $message(msg, 'error')
